"""Parser for .atf (Agentic Test Format) files"""
...
class ATFScenario:
    """Represents a single test scenario from an ATF file"""
    
    def __init__(self, name: str, steps: str, tags: List[str] = None):
        self.name = name
        self.steps = steps
        self.tags = tags or []
...
class ATFTestSuite:
    """Represents a complete ATF test file"""
    ...
    def __init__(
        ...
        description: str = "",
        config: Dict = None,
        scenarios: List[ATFScenario] = None
    ):
        self.name = name
...
class ATFParser:
    ...
    def parse_file(self, file_path: str) -> ATFTestSuite:
        """Parse an ATF file and return a test suite
        
        Args:
            file_path: Path to .atf file
            
        Returns:
            ATFTestSuite object containing all scenarios
        """
        path = Path(file_path)
        if not path.exists():
            raise FileNotFoundError(f"ATF file not found: {file_path}")
        
        content = path.read_text()
        return self.parse_content(content)
    
    def parse_content(self, content: str) -> ATFTestSuite:
        """Parse ATF content from string
        
        Args:
            content: ATF file content as string
    ...
    def validate(self, test_suite: ATFTestSuite) -> List[str]:
        """Validate a test suite and return list of warnings/errors
        
        Args:
            test_suite: ATFTestSuite to validate
